...
- ~~[PRODUCT_IMPLEMENTATION_DESIGN.ko.md](PRODUCT_IMPLEMENTATION_DESIGN.ko.md)~~ → [deprecated](deprecated/PRODUCT_IMPLEMENTATION_DESIGN.ko.md)
- ~~[DOMAIN_MODEL.ko.md](DOMAIN_MODEL.ko.md)~~ → [deprecated](deprecated/DOMAIN_MODEL.ko.md)
- ~~[PLACEMENT_AND_FALLBACK_POLICY.ko.md](PLACEMENT_AND_FALLBACK_POLICY.ko.md)~~ → [deprecated](deprecated/PLACEMENT_AND_FALLBACK_POLICY.ko.md)
- ~~[DRAGONFLY_ADAPTER_SPEC.ko.md](DRAGONFLY_ADAPTER_SPEC.ko.md)~~ → scope에서 제거, [deprecated](deprecated/DRAGONFLY_ADAPTER_SPEC.ko.md) 참조

## 2. 아키텍처적 위치

`artifact-handoff`는 Kubernetes batch integration용 long-lived resolver service로 만들어져야 한다.
...
Resolver-service layer는 주요 orchestrator다.

책임:

- product state transition 처리
...
Resolver-service layer는 다음을 하면 안 된다.
