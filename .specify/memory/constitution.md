...
artifact-handoff does **not** treat NodeVault §4 as an independent platform
...
4. every scoped/domain/component authority required by the artifact-handoff task
   is explicitly present in the task `Authority Snapshot`;
...
artifact-handoff work that depends on platform structure/ownership/call-direction
or the resolution contract must carry the exact CURRENT architecture and the
relevant scoped/component contract directly in the task `Authority Snapshot`.

Note: the artifact-handoff **resolution contract** (`PlacementIntent` /
`MaterializationPlan`, and the `MISSING` / policy / digest / GC status axis) is
consumed by JUMI and is therefore **cross-repo** — it is not declared here.
Its authoritative home is the scoped platform/component contract selected by the
current Authority Snapshot, not this constitution or the invariant mirror.
...
- **race tests** (IMPLEMENTED — `make test` race variant in CI): concurrency safety.

## §1.10 — "do not record what you did not observe"

**Authority: CURRENT platform invariant under `AR-2026-08-17.1`. Enforcement in
this repo: PROPOSED where no deterministic local gate exists.** artifact-handoff
has no deterministic rule that generally enforces this invariant today. The
